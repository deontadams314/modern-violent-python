...
def nmapScan(target, port):
    ...
    try:
        ...
        nmScan.scan(target, port, arguments="-sV")
        state = nmScan[target]['tcp'][int(port)]['state']
        service = nmScan[target]['tcp'][int(port)].get('name', '')
        version = nmScan[target]['tcp'][int(port)].get('version', '')
        result = f"[*] {target} TCP/{port} = {state} | Service: {service} {version}"
        return result
    except:
...
def main():
    ...
    try:
        with ThreadPoolExecutor(max_workers=args.threads) as executor:
            futures = [executor.submit(nmapScan, resolved_ip, port) for port in ports]
